...
int seed_ali()
{
	...
	{
		start = clock();
		load_input_sorted_vcf();
		end = clock();

		t = (double)(end - start) / CLOCKS_PER_SEC;

		fprintf(stderr, "%lf seconds is used for loading vcf-info index\n", t);		
	}
	...
	if(tree_flag)
	{
		fprintf(stderr, "begin allocating memory for externsion tree and LV queue\n");

		seqss1 = (uint8_t** )calloc(thread_n, sizeof(uint8_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((seqss1[r_i] = (uint8_t* )calloc(MAX_EXT_ALT_POS * readlen_max, 1)) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for seqss1\n");
				exit(2);
			}

		seqss2 = (uint8_t** )calloc(thread_n, sizeof(uint8_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((seqss2[r_i] = (uint8_t* )calloc(MAX_EXT_ALT_POS * readlen_max, 1)) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for seqss1\n");
				exit(2);
			}

		node_arrays = (treenode*** )calloc(thread_n, sizeof(treenode** ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((node_arrays[r_i] = (treenode** )calloc(NODES_MAX, sizeof(treenode* ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for node_arrays\n");
				exit(2);
			}


		seq_nodes = (uint16_t*** )calloc(thread_n, sizeof(uint16_t** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			seq_nodes[r_i] = (uint16_t** )calloc(MAX_EXT_ALT_POS, sizeof(uint16_t* ));
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
			{
				if((seq_nodes[r_i][m] = (uint16_t* )calloc(readlen_max, sizeof(uint16_t ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for seq_nodes\n");
					exit(2);
				}
			}
		}

		seq_node_ns = (uint16_t** )calloc(thread_n, sizeof(uint16_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((seq_node_ns[r_i] = (uint16_t* )calloc(MAX_EXT_ALT_POS, sizeof(uint16_t ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for seq_node_n\n");
				exit(2);
			}
		}

		seq_node_lens = (uint16_t*** )calloc(thread_n, sizeof(uint16_t** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			seq_node_lens[r_i] = (uint16_t** )calloc(MAX_EXT_ALT_POS, sizeof(uint16_t* ));
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
			{
				if((seq_node_lens[r_i][m] = (uint16_t* )calloc(readlen_max, sizeof(uint16_t ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for seq_node_lens\n");
					exit(2);
				}
			}
		}

		seq_node_len_ns = (uint16_t** )calloc(thread_n, sizeof(uint16_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((seq_node_len_ns[r_i] = (uint16_t* )calloc(MAX_EXT_ALT_POS, sizeof(uint16_t ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for seq_node_len_ns\n");
				exit(2);
			}
		}

		alt_info_arrays = (alt_info_a** )calloc(thread_n, sizeof(alt_info_a* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((alt_info_arrays[r_i] = (alt_info_a* )calloc(MAX_EXT_ALT_POS *  readlen_max, sizeof(alt_info_a))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for alt_info_arrays\n");
				exit(2);
			}
		}

		alt_seqss = (alt_seq_a** )calloc(thread_n, sizeof(alt_seq_a* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((alt_seqss[r_i] = (alt_seq_a* )calloc(MAX_EXT_ALT_POS * readlen_max , sizeof(alt_seq_a))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for alt_seqss\n");
				exit(2);
			}
		}

		route_seps = (uint16_t** )calloc(thread_n, sizeof(uint16_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((route_seps[r_i] = (uint16_t* )calloc(MAX_EXT_ALT_POS + 2, sizeof(uint16_t ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for route_seps\n");
				exit(2);
			}
		}

		//for LVs
		lv_queues = (ext_queue*** )calloc(thread_n, sizeof(ext_queue** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			lv_queues[r_i] = (ext_queue** )calloc(NODES_QUEUE_MAX, sizeof(ext_queue* ));
			for(m = 0; m < NODES_QUEUE_MAX; m++)
			{
				if((lv_queues[r_i][m] = (ext_queue* )calloc(1, sizeof(ext_queue ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for lv_queues\n");
					exit(2);
				}
			}
		}

		lv_results = (ext_result*** )calloc(thread_n, sizeof(ext_result** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			lv_results[r_i] = (ext_result** )calloc(MAX_EXT_ALT_POS, sizeof(ext_result* ));
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
				if((lv_results[r_i][m] = (ext_result* )calloc(1, sizeof(ext_result ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for lv_results\n");
					exit(2);
				}
		}

		rnode_array = (route_node*** )calloc(thread_n, sizeof(route_node** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			rnode_array[r_i] = (route_node** )calloc(MAX_ROUTE_NUM, sizeof(route_node* ));
			for(m = 0; m < MAX_ROUTE_NUM; m++)
				if((rnode_array[r_i][m] = (route_node* )calloc(1, sizeof(route_node ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for rnode_array\n");
					exit(2);
				}
		}

		routeid_arrays = (uint32_t*** )calloc(thread_n, sizeof(uint32_t** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			routeid_arrays[r_i] = (uint32_t** )calloc((MAX_K_VAR + 2) * MAX_K_VAR, sizeof(uint32_t* ));//(MAX_K_VAR << 1) + 2
			for(m = 0; m < (MAX_K_VAR + 2) * MAX_K_VAR; m++)//(MAX_K_VAR << 1) + 2
				if((routeid_arrays[r_i][m] = (uint32_t* )calloc(MAX_ROUTE_NUM, 4)) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for routeid_arrays\n");
					exit(2);
				}
		}

		route_id_marks = (uint32_t** )calloc(thread_n, sizeof(uint32_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			route_id_marks[r_i] = (uint32_t* )calloc(MAX_ROUTE_NUM, 4);
			if(route_id_marks[r_i] == NULL)	exit(2);
		}

		route_id_marks_pre = (uint32_t** )calloc(thread_n, sizeof(uint32_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			route_id_marks_pre[r_i] = (uint32_t* )calloc(MAX_ROUTE_NUM, 4);
			if(route_id_marks_pre[r_i] == NULL)	exit(2);
		}

		ex_results1 = (ext_results** )calloc(thread_n, sizeof(ext_results* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((ex_results1[r_i] = (ext_results* )calloc(TREE_EXT_OUTPUT, sizeof(ext_results ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for ex_results\n");
				exit(2);
			}

		ex_results2 = (ext_results** )calloc(thread_n, sizeof(ext_results* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((ex_results2[r_i] = (ext_results* )calloc(TREE_EXT_OUTPUT, sizeof(ext_results ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for ex_results\n");
				exit(2);
			}

		c_results1 = (com_result** )calloc(thread_n, sizeof(com_result* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((c_results1[r_i] = (com_result* )calloc(TREE_EXT_OUTPUT, sizeof(com_result ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for c_results1\n");
				exit(2);
			}

		c_results2 = (com_result** )calloc(thread_n, sizeof(com_result* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((c_results2[r_i] = (com_result* )calloc(TREE_EXT_OUTPUT, sizeof(com_result ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for c_results2\n");
				exit(2);
			}

		ref_seq_ori = (uint8_t*** )calloc(thread_n, sizeof(uint8_t** ));

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((ref_seq_ori[r_i] = (uint8_t** )calloc(MAX_EXT_ALT_POS, sizeof(uint8_t* ))) == NULL)	exit(1);

			for(m = 0; m < MAX_EXT_ALT_POS; m++)
			{
				if((ref_seq_ori[r_i][m] = (uint8_t* )calloc(readlen_max, 1)) == NULL)
					exit(1);
			}
		}

		ref_seq_var = (uint8_t*** )calloc(thread_n, sizeof(uint8_t** ));

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			if((ref_seq_var[r_i] = (uint8_t** )calloc(MAX_EXT_ALT_POS, sizeof(uint8_t* ))) == NULL)	exit(1);

			for(m = 0; m < MAX_EXT_ALT_POS; m++)
			{
				if((ref_seq_var[r_i][m] = (uint8_t* )calloc(readlen_max, 1)) == NULL)
					exit(1);
			}
		}

		cigar_ps = (char** )calloc(thread_n, sizeof(char* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((cigar_ps[r_i] = (char* )calloc(MAX_LV_CIGARCOM, sizeof(char ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for cigar_ps\n");
				exit(2);
			}

		f_cigars = (char** )calloc(thread_n, sizeof(char* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((f_cigars[r_i] = (char* )calloc(MAX_LV_CIGARCOM, sizeof(char ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for f_cigar\n");
				exit(2);
			}

		b_cigars = (char** )calloc(thread_n, sizeof(char* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((b_cigars[r_i] = (char* )calloc(MAX_LV_CIGARCOM, sizeof(char ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for b_cigar\n");
				exit(2);
			}
			
		str_os = (char** )calloc(thread_n, sizeof(char* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			if((str_os[r_i] = (char* )calloc(MAX_LV_CIGARCOM, sizeof(char ))) == NULL)
			{
				fprintf(stderr, "Error of allocating momery for b_cigar\n");
				exit(2);
			}
			
		ext_out1s = (ext_output*** )calloc(thread_n, sizeof(ext_output** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			ext_out1s[r_i] = (ext_output** )calloc(TREE_EXT_OUTPUT, sizeof(ext_output* ));
			for(m = 0; m < TREE_EXT_OUTPUT; m++)
			{
				if((ext_out1s[r_i][m] = (ext_output* )calloc(1, sizeof(ext_output ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for ext_out1s\n");
					exit(2);
				}
			}
		}
		
		ext_out2s = (ext_output*** )calloc(thread_n, sizeof(ext_output** ));
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			ext_out2s[r_i] = (ext_output** )calloc(TREE_EXT_OUTPUT, sizeof(ext_output* ));
			for(m = 0; m < TREE_EXT_OUTPUT; m++)
			{
				if((ext_out2s[r_i][m] = (ext_output* )calloc(1, sizeof(ext_output ))) == NULL)
				{
					fprintf(stderr, "Error of allocating momery for ext_out2s\n");
					exit(2);
				}
			}
		}
		
		mat_index1s = (uint16_t** )calloc(thread_n, sizeof(uint16_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			mat_index1s[r_i] = (uint16_t* )calloc(CUS_SEED_SET, 2);
	
		mat_index2s = (uint16_t** )calloc(thread_n, sizeof(uint16_t* ));
		for(r_i = 0; r_i < thread_n; r_i++)
			mat_index2s[r_i] = (uint16_t* )calloc(CUS_SEED_SET, 2);
	...
	if(flag_std)
	{
		fprintf(stderr, "@RG\tID:L1\tPU:SC_1_10\tLB:SC_1\tSM:NA12878\n");
		fprintf(stderr, "@RG\tID:L2\tPU:SC_2_12\tLB:SC_2\tSM:NA12878\n");
	}
	else
	{
	...
	while ((kr1 > 0) && (kr2 > 0))
	{
		...
		else
		{
			...
			//debug
			//fprintf(stderr, "out worker seqi: %u qual: %u cigar: %s\n", seqi, seqio[seqi].qualc1, seqio[seqi].cigar1);
		...
		if(flag_std)
		{
			...
			for(seqi = 0; seqi < seqii; seqi++)
			{
				...
				if(seqio[seqi].length_h1 == 0)
				{
					...
					{
						fprintf(stderr, "%s\t%u\t%s\t%"PRId64"\t%d\t%s\t%c\t%"PRId64"\t%"PRId64"\t%s\t%s",
					...
					{
						fprintf(stderr, "%s\t%u\t%s\t%"PRId64"\t%d\t%s\t%c\t%"PRId64"\t0\t%s\t%s",
						       seqio[seqi].name, seqio[seqi].flag1, chr_names[seqio[seqi].chr_re1], seqio[seqi].pos1,
					...
					if(seqio[seqi].xa_n_p1 > 0)
					{
						...
						for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n_p1; v_cnt_i++)
							fprintf(stderr, "%s,%c%u,%s,%d;",chr_names[seqio[seqi].chr_res[v_cnt_i]], seqio[seqi].xa_d1s[v_cnt_i], seqio[seqi].sam_pos1s[v_cnt_i], seqio[seqi].cigar_p1s[v_cnt_i], seqio[seqi].lv_re1s[v_cnt_i]);
					}

#ifdef	PR_SINGLE
					if(seqio[seqi].xa_n1 > 0)
					{
						if((seqio[seqi].xa_n_p1 == 0) && (seqio[seqi].xa_n1 > 0))	fprintf(stderr, "\tXA:Z:");
						for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n1; v_cnt_i++)
							fprintf(stderr, "%s,%c%u,%uM,%u;", chr_names[seqio[seqi].chr_res1[v_cnt_i]], seqio[seqi].xa_ds1[v_cnt_i], seqio[seqi].sam_poss1[v_cnt_i], seqio[seqi].read_length1, seqio[seqi].lv_res1[v_cnt_i]);
					...
					if(seqio[seqi].xa_n_x1 > 0)
					{
						...
						{
							fprintf(stderr, "%s,%u,%c,%s,%u,%d;", chr_names[seqio[seqi].chr_res_s1[v_cnt_i]], seqio[seqi].sam_pos1s[v_cnt_i + xa_n_p1_tmp], seqio[seqi].xa_d1s[v_cnt_i + xa_n_p1_tmp], seqio[seqi].cigar_p1s[v_cnt_i + xa_n_p1_tmp], seqio[seqi].qualc1, seqio[seqi].lv_re1s[v_cnt_i + xa_n_p1_tmp]);
					...
					fprintf(stderr, "\tNM:i:%u\tRG:Z:L1\n", seqio[seqi].nm1);

#else

					fprintf(stderr, "\tNM:i:%u\n", seqio[seqi].nm1);
				...
				else
				{
					...
					if(seqio[seqi].chr_re1 == seqio[seqi].chr_re2)
					{
						fprintf(stderr, "%s\t%u\t%s\t%"PRId64"\t%d\t%s%s\t%c\t%"PRId64"\t%"PRId64"\t%s\t%s",
						       seqio[seqi].name, seqio[seqi].flag1, chr_names[seqio[seqi].chr_re1], seqio[seqi].pos1,
					...
					{
						fprintf(stderr, "%s\t%u\t%s\t%"PRId64"\t%d\t%s%s\t%c\t%"PRId64"\t0\t%s\t%s",
						       seqio[seqi].name, seqio[seqi].flag1, chr_names[seqio[seqi].chr_re1], seqio[seqi].pos1,
					...
					if(seqio[seqi].xa_n_p1 > 0)
					{
						...
						for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n_p1; v_cnt_i++)
						{
							fprintf(stderr, "%s,%c%u,%s%s,%d;",chr_names[seqio[seqi].chr_res[v_cnt_i]], seqio[seqi].xa_d1s[v_cnt_i], seqio[seqi].sam_pos1s[v_cnt_i], seqio[seqi].cigar_p1s[v_cnt_i], h_chars, seqio[seqi].lv_re1s[v_cnt_i]);
						}

					}
					fprintf(stderr, "NM:i:%u\t", seqio[seqi].nm1);
				...
				for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n_x1; v_cnt_i++)
				{
					...
					fprintf(stderr, "%s\t%u\t%s\t%"PRId64"\t%d\t%s\t%c\t%"PRId64"\t%d\t%s\t%s\tNM:i:%u\tRG:Z:L1\n",
				...
				if(seqio[seqi].length_h2 == 0)
				{
					...
					if(seqio[seqi].xa_n_p2 > 0)
					{
						...
						for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n_p2; v_cnt_i++)
							fprintf(stderr, "%s,%c%u,%s,%d;",chr_names[seqio[seqi].chr_res[v_cnt_i]], seqio[seqi].xa_d2s[v_cnt_i], seqio[seqi].sam_pos2s[v_cnt_i], seqio[seqi].cigar_p2s[v_cnt_i], seqio[seqi].lv_re2s[v_cnt_i]);

					}
#ifdef	PR_SINGLE
					if(seqio[seqi].xa_n2 > 0)
					{
						if((seqio[seqi].xa_n_p2 == 0) && (seqio[seqi].xa_n2 > 0))	fprintf(stderr, "\tXA:Z:");
						for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n2; v_cnt_i++)
							fprintf(stderr, "%s,%c%u,%uM,%u;", chr_names[seqio[seqi].chr_res2[v_cnt_i]], seqio[seqi].xa_ds2[v_cnt_i], seqio[seqi].sam_poss2[v_cnt_i], seqio[seqi].read_length2, seqio[seqi].lv_res2[v_cnt_i]);
					...
					if(seqio[seqi].xa_n_x2 > 0)
					{
						...
						{
							fprintf(stderr, "%s,%u,%c,%s,%u,%d;", chr_names[seqio[seqi].chr_res_s2[v_cnt_i]], seqio[seqi].sam_pos2s[v_cnt_i + xa_n_p1_tmp], seqio[seqi].xa_d2s[v_cnt_i + xa_n_p1_tmp], seqio[seqi].cigar_p2s[v_cnt_i + xa_n_p1_tmp], seqio[seqi].qualc2, seqio[seqi].lv_re2s[v_cnt_i + xa_n_p1_tmp]);
				...
				else
				{
					...
					if(seqio[seqi].xa_n_p2 > 0)
					{
						...
						for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n_p2; v_cnt_i++)
						{
							fprintf(stderr, "%s,%c%u,%s%s,%d;",chr_names[seqio[seqi].chr_res[v_cnt_i]], seqio[seqi].xa_d2s[v_cnt_i], seqio[seqi].sam_pos2s[v_cnt_i], seqio[seqi].cigar_p2s[v_cnt_i], h_chars, seqio[seqi].lv_re2s[v_cnt_i]);
						}
					}
#ifdef	PR_SINGLE
					if(seqio[seqi].xa_n2 > 0)
				...
				for(v_cnt_i = 0; v_cnt_i < seqio[seqi].xa_n_x2; v_cnt_i++)
				{
					...
					fprintf(stderr, "%s\t%u\t%s\t%"PRId64"\t%d\t%s\t%c\t%"PRId64"\t%d\t%s\t%s\tNM:i:%u\tRG:Z:L2\n",
		...
		else
		{
			...
			for(seqi = 0; seqi < seqii; seqi++)
			{
				...
				if(seqio[seqi].length_h1 == 0)
				{
					if(seqio[seqi].chr_re1 == seqio[seqi].chr_re2)
					{
						//fprintf(stderr, "seqi: %u qual: %u cigar: %s\n", seqi, seqio[seqi].qualc1, seqio[seqi].cigar1);
	...
	if(tree_flag)
	{
		...
		{
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
				if(ref_seq_ori[r_i][m])	free(ref_seq_ori[r_i][m]);
			if(ref_seq_ori[r_i])	free(ref_seq_ori[r_i]);
		}
		if(ref_seq_ori)	free(ref_seq_ori);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
				if(ref_seq_var[r_i][m])	free(ref_seq_var[r_i][m]);
			if(ref_seq_var[r_i])	free(ref_seq_var[r_i]);
		}
		if(ref_seq_var)	free(ref_seq_var);
		if(rpos_array) free(rpos_array);
		if(aindex_array) free(aindex_array);
		if(apos_array) free(apos_array);
		if(rpos_end_array) free(rpos_end_array);
		if(aseq_array)	free(aseq_array);
		if(ref_seqv_array) free(ref_seqv_array);
		if(type_array) free(type_array);

		if(rpos_array_rec) free(rpos_array_rec);
		if(aindex_array_rec) free(aindex_array_rec);
		if(apos_array_rec) free(apos_array_rec);
		if(rpos_end_array_rec) free(rpos_end_array_rec);
		if(aseq_array_rec)	free(aseq_array_rec);
		if(ref_seqv_array_rec) free(ref_seqv_array_rec);
		if(type_array_rec) free(type_array_rec);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(seqss1[r_i])	free(seqss1[r_i]);
		if(seqss1)	free(seqss1);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(seqss2[r_i])	free(seqss2[r_i]);
		if(seqss2)	free(seqss2);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(node_arrays[r_i])	free(node_arrays[r_i]);
		if(node_arrays)	free(node_arrays);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
				if(seq_nodes[r_i][m])	free(seq_nodes[r_i][m]);
			if(seq_nodes[r_i])	free(seq_nodes[r_i]);
		}
		if(seq_nodes)	free(seq_nodes);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(seq_node_ns[r_i])	free(seq_node_ns[r_i]);
		if(seq_node_ns)	free(seq_node_ns);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
				if(seq_node_lens[r_i][m])	free(seq_node_lens[r_i][m]);
			if(seq_node_lens[r_i])	free(seq_node_lens[r_i]);
		}
		if(seq_node_lens)	free(seq_node_lens);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(seq_node_len_ns[r_i])	free(seq_node_len_ns[r_i]);
		if(seq_node_len_ns)	free(seq_node_len_ns);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(alt_info_arrays[r_i])	free(alt_info_arrays[r_i]);
		if(alt_info_arrays)	free(alt_info_arrays);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(alt_seqss[r_i])	free(alt_seqss[r_i]);
		if(alt_seqss)	free(alt_seqss);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(route_seps[r_i])	free(route_seps[r_i]);
		if(route_seps)	free(route_seps);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < NODES_QUEUE_MAX; m++)
				if(lv_queues[r_i][m])	free(lv_queues[r_i][m]);
			if(lv_queues[r_i])	free(lv_queues[r_i]);
		}
		if(lv_queues)	free(lv_queues);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < MAX_EXT_ALT_POS; m++)
				if(lv_results[r_i][m])	free(lv_results[r_i][m]);
			if(lv_results[r_i])	free(lv_results[r_i]);
		}
		if(lv_results)	free(lv_results);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < MAX_ROUTE_NUM; m++)
				if(rnode_array[r_i][m])	free(rnode_array[r_i][m]);
			if(rnode_array[r_i])	free(rnode_array[r_i]);
		}
		if(rnode_array)	free(rnode_array);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < (MAX_K_VAR + 2) * MAX_K_VAR; m++)//(MAX_K_EXT << 1) + 2
				if(routeid_arrays[r_i][m])	free(routeid_arrays[r_i][m]);

			if(routeid_arrays[r_i])	free(routeid_arrays[r_i]);
		}
		if(routeid_arrays)	free(routeid_arrays);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(route_id_marks[r_i])	free(route_id_marks[r_i]);
		if(route_id_marks)	free(route_id_marks);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(route_id_marks_pre[r_i])	free(route_id_marks_pre[r_i]);
		if(route_id_marks_pre)	free(route_id_marks_pre);
		
		for(r_i = 0; r_i < thread_n; r_i++)
			if(ex_results1[r_i])	free(ex_results1[r_i]);
		if(ex_results1)	free(ex_results1);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(ex_results2[r_i])	free(ex_results2[r_i]);
		if(ex_results2)	free(ex_results2);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(c_results1[r_i])	free(c_results1[r_i]);
		if(c_results1)	free(c_results1);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(c_results2[r_i])	free(c_results2[r_i]);
		if(c_results2)	free(c_results2);

		for(r_i = 0; r_i < thread_n; r_i++)
			if(cigar_ps[r_i])	free(cigar_ps[r_i]);
		if(cigar_ps)	free(cigar_ps);

		/*
		for(r_i = 0; r_i < thread_n; r_i++)
			if(f_cigars[r_i])	free(f_cigars[r_i]);
		if(f_cigars)	free(f_cigars);
		*/
		
		for(r_i = 0; r_i < thread_n; r_i++)
			if(b_cigars[r_i])	free(b_cigars[r_i]);
		if(b_cigars)	free(b_cigars);
		
		for(r_i = 0; r_i < thread_n; r_i++)
			if(str_os[r_i])	free(str_os[r_i]);
		if(str_os)	free(str_os);

		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < TREE_EXT_OUTPUT; m++)
				if(ext_out1s[r_i][m])	free(ext_out1s[r_i][m]);

			if(ext_out1s[r_i])	free(ext_out1s[r_i]);
		}
		if(ext_out1s)	free(ext_out1s);
		
		for(r_i = 0; r_i < thread_n; r_i++)
		{
			for(m = 0; m < TREE_EXT_OUTPUT; m++)
				if(ext_out2s[r_i][m])	free(ext_out2s[r_i][m]);

			if(ext_out2s[r_i])	free(ext_out2s[r_i]);
		}
		if(ext_out2s)	free(ext_out2s);
		
		for(r_i = 0; r_i < thread_n; r_i++)
			if(mat_index1s[r_i])	free(mat_index1s[r_i]);
		if(mat_index1s)	free(mat_index1s);
		
		for(r_i = 0; r_i < thread_n; r_i++)
			if(mat_index2s[r_i])	free(mat_index2s[r_i]);
		if(mat_index2s)	free(mat_index2s);
	}
	...
	if(chr_end_n)	free(chr_end_n);
	if(chr_end_n_rec)	free(chr_end_n_rec);
...
#endif
{
...
#endif
	{
		...
		if((de_m_p_o[tid] == 1) && tree_flag)
		{
			ext_flag = extension_tree_alt(seed_pr, spa_i, unmatch, seed_set_pos, buffer_ref_seq, ref_seqv_array, ref_seq_ori[tid], ref_seq_var[tid], read_char[tid], read_bit1[tid], read_bit2[tid], read_length1, read_length2, tid, lv_k1, lv_k2, seqi, (int )devi, rh[tid], anchor_hash[tid], anchor_array[tid], anchor_point[tid], anchor_pos[tid]);
		}

		//deal with unmatched reads
#ifdef	UNMATCH_SINGLE_END
		if((de_m_p_o[tid] == 1) && ((!tree_flag) || (tree_flag && (!ext_flag) && anchor_ext_flag)))
		{
			...
			for(rc_i = 0; rc_i < 2; rc_i++)
			{
				for(un_ii = 0; un_ii < 2; un_ii++)
				{
...
#ifdef	DEBUG_EXT_ALN_W	
					fprintf(stderr, "seed info: %u %u %u\n", rc_i, un_ii, spa_i[rc_i][un_ii]);
#endif	
...
#ifdef	DEBUG_EXT_ALN_W	
							//debug
							int debug_i = 0, low = 0, high = 0, mid = 0, chr_re = 0;
							uint32_t debug_pos = 0;
					...
					{
						mid = (low + high) >> 1;
						if(debug_pos < (chr_end_n[mid]))
						{
							high = mid - 1;
						}
						else if(debug_pos > (chr_end_n[mid]))
						{
							low = mid + 1;
						}
						else
					...
					fprintf(stderr, "%u %u\n", chr_re, debug_pos - chr_end_n[chr_re - 1] + 1);
#endif
...
#ifdef	EXT_ANCHOR
			fprintf(stderr, "chr: %u pos: %u anchor_hash_i: %u ksw_s: %u ksw_e: %u k_anchor: %u\n", chr_re, posi, anchor_hash_i, ksw_s, ksw_e, k_anchor);
#endif	

#ifdef	EXT_ANCHOR
				fprintf(stderr, "read_bit_2: %u %u %u\n", read_bit_2[tid][0], read_bit_2[tid][1], read_bit_2[tid][2]);
#endif
				for(base_i = ksw_s - 1; base_i < ksw_e - k_anchor; base_i += anchor_seed_d)
				{
...
#ifdef	EXT_ANCHOR
					fprintf(stderr, "anchor_ref: %u anchor_hash_p[anchor_ref >> 4]: %u base_i: %u r_b_v: %u\n", anchor_ref, anchor_hash_p[anchor_ref >> 4], base_i, r_b_v);
#endif	
					max_right = 0;
					for(tra_i = 0; tra_i < (anchor_hash_p[anchor_ref >> 4] & 0X1f); tra_i++)
					{
						...
						{
#ifdef	EXT_ANCHOR					
							if(anchor_hash_p[anchor_ref >> 4] == 1761)
								fprintf(stderr, "detail: index %u point_p[array_index]: %u pos_p: %u bp_s: %u bp_e: %u\n", array_index, anchor_point_p[array_index], anchor_pos_p[anchor_point_p[array_index]], read_bit_2[tid][(anchor_pos_p[anchor_point_p[array_index]] - 1) >> 5],read_bit_2[tid][(anchor_pos_p[anchor_point_p[array_index]] + k_anchor) >> 5] );
#endif	
							max_seed_length = 0;
...
#ifdef	EXT_ANCHOR
					fprintf(stderr, "s_r_o_l: %d s_r_o_r: %u base_i_off_l: %u base_i_off_r: %u max_seed_length: %u\n\n", left_i, right_i, base_i_off_l, base_i_off_r, max_seed_length);
#endif	
					d_n1 = 0;
					i_n1 = 0;
...
#ifdef	EXT_ANCHOR
						uint32_t debug_i = 0;
						for(debug_i = 0; debug_i < read_b_i; debug_i++)
							fprintf(stderr, "%c", Dna5Tochar[ali_ref_seq[tid][debug_i]]);
						fprintf(stderr, "\n");
#endif
				...
				if(op_rc[tid][v_cnt_i] == 0)
				{
					...
					if(other_end_flag)
					{
						...
						//if((rcs == 0) || (rcs == 3))	fprintf(stderr, "Error: %u\n", rcs);
				...
				else if(op_rc[tid][v_cnt_i] == 1)
				{
					...
					if(other_end_flag)
					{
						...
						//if((rcs == 1) || (rcs == 2))	fprintf(stderr, "Error: %u\n", rcs);
...
/*
	...
	fprintf(stderr, "ori seed_set_pos:\n");
	
	for(debug_i = 0; debug_i < set_pos_n; debug_i++)
	{
			debug_pos = (*seed_set_pos)[debug_i];
					low = 0;
					high = chr_file_n - 1;
					while ( low <= high )
					{
...
#ifdef	DEBUG_EXT_ALN
	fprintf(stderr, "enter repetitive\n");
#endif
...
int seed_ali_single_end(int argc, char *argv[])
{
	...
	fprintf(stderr, "\n%lf seconds is used for loading index tree_flag: %u\n", t, tree_flag);
